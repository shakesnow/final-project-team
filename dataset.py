...
class MEFdataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if lr_over is None or lr_under is None or hr_over is None or hr_under is None or hr is None:
            raise ValueError(f"Error reading images at index {idx}")
        ...
        lr_over_p, lr_under_p, hr_over_p, hr_under_p, hr_p = self.get_patch(lr_over,
                                                                            ...
                                                                            hr_under,
                                                                            hr)
        # Check if patch sizes are correct
        if lr_over_p.shape != (self.patch_size, self.patch_size, 3) or lr_under_p.shape != (
        self.patch_size, self.patch_size, 3) or hr_over_p.shape[:2] != (
        self.patch_size * self.scale, self.patch_size * self.scale) or hr_under_p.shape[:2] != (
        self.patch_size * self.scale, self.patch_size * self.scale) or hr_p.shape[:2] != (
        self.patch_size * self.scale, self.patch_size * self.scale):
            raise ValueError(f"Incorrect patch size at index {idx}")
