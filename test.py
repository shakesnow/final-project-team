    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx in trange(self.num_imgs):
                ...
                img_fused_png = img_fused.astype(np.uint8)
                cv2.imwrite(os.path.join(args.save_dir, str(save_name) + args.ext), img_fused_png)
                # # Convert to float32 for HDR
                # img_fused = img_fused.astype(np.float32)
                # # Save as HDR
                # cv2.imwrite(os.path.join(args.save_dir, str(save_name) + '.hdr'), img_fused)
                # Save as HDR
                # Normalize and Save as HDR
                img_fused_hdr = img_fused / np.max(img_fused)  # Normalize to [0, man] range
                print(
                    f'Image {save_name} range after normalization: min {img_fused_hdr.min()}, max {img_fused_hdr.max()}')
                cv2.imwrite(os.path.join(args.save_dir, str(save_name) + '.hdr'), img_fused_hdr)
